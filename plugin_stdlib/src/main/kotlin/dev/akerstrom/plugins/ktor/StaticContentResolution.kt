...
public fun ApplicationCall.resolveResource(
    ...
    val resolveContent: (URL) -> OutgoingContent.ReadChannelContent? = { url ->
        resourceClasspathResource(url, normalizedPath) { mimeResolve(it.path.extension()) }
    }

    resourceClass?.getResource("/$normalizedPath")?.let(resolveContent)?.let { content ->
        return content
    }

    for (url in classLoader.getResources(normalizedPath).asSequence()) {
        resolveContent(url)?.let { content ->
            return content
        }
    }
...
internal fun Application.resolveResource(
    ...
    val normalizedPath = normalisedPath(resourcePackage, path)
    val rcCacheKey = "${resourceClass.hashCode()}/$normalizedPath"
    val clCacheKey = "${classLoader.hashCode()}/$normalizedPath"
    ...
    fun resourceWithCache(cacheKey: String, block: () -> Pair<URL, OutgoingContent.ReadChannelContent>?): Pair<URL, OutgoingContent.ReadChannelContent>? {
        return resourceCache[cacheKey]?.let(resolveContent) ?: block()
            ?.also { (url) -> resourceCache[rcCacheKey] = url }
    }

    resourceClass?.let { rc ->
        resourceWithCache(rcCacheKey) {
            rc.getResource("/$normalizedPath")?.let(resolveContent)
        }
    }?.let { return it }

    return resourceWithCache(clCacheKey) {
        classLoader.getResources(normalizedPath).asSequence().firstNotNullOfOrNull(resolveContent)
    }
}
